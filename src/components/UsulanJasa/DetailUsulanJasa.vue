<template>
    ...
    <v-row class="mt-2">
        <v-col cols="12">
            ...
            <v-dialog v-model="dialogKonfirmasi" :overlay="false" max-width="500px" transition="dialog-transition">
                <v-card class="pb-4">
                    ...
                    <v-card-text class="mt-4">
                        <v-text-field v-model="keteranganKonfirmasi" label="Masukkan Keterangan Konfirmasi" outlined
                            dense></v-text-field>
                        <div class="d-flex justify-center">
                            <v-btn @click="updateKonfirmasi(statusKonfirmasi)"
                                :color="statusKonfirmasi ? 'success' : 'warning'" rounded>
                                <v-icon class="mr-4">mdi-content-save
                                </v-icon> Simpan
                            </v-btn>
                            <v-spacer></v-spacer>
                            <v-btn color="error" @click="dialogKonfirmasi = false" outlined rounded>
                            ...
                            <v-simple-table dense>
                                <tbody>
                                    <tr>
                                        ...
                                        <td width="10%">:</td>
                                        <td width="60%">{{ detail.items[0].no_usulan }}</td>
                                    </tr>
                                    <tr>
                                        <td>Tgl. Usulan</td>
                                        <td>:</td>
                                        <td>{{ getFormatTanggal(detail.items[0].tanggal_usulan) }}</td>
                                    </tr>
                                    <tr>
                                        <td>Pembuat</td>
                                        <td>:</td>
                                    ...
                                    <tr>
                                        ...
                                        <td>:</td>
                                        <td>{{ detail.items[0].nama_bagian }} / {{ detail.items[0].nama_dept }} </td>
                                    </tr>
                                    <tr>
                                        <td>Tgl. Kirim Dokumen</td>
                                        <td>:</td>
                                        <td>{{ getFormatTanggal(detail.items[0].tanggal_kirim_dok) }}</td>
                                    </tr>
                                    <tr>
                                        <td>Tgl. Terima Dokumen</td>
                                        <td>:</td>
                                        <td>{{ getFormatTanggal(detail.items[0].tanggal_terima_dok) }}</td>
                                    </tr>
                                </tbody>
                            </v-simple-table>
                        </v-col>
                            ...
                            <v-simple-table dense>
                                <tbody>
                                    <tr>
                                        ...
                                        <td width="10%">:</td>
                                        <td width="60%">{{ getFormatTanggal(detail.items[0].tanggal_kirim_konf) }} - {{
                                    ...
                                    <tr>
                                        ...
                                        <td>:</td>
                                        <td>{{ getFormatTanggal(detail.items[0].kabag_tgl) }} -  {{ detail.items[0].kabag_nik }} </td>
                                    </tr>
                                    <tr>
                                        <td>Konfirmasi Manajer</td>
                                        <td>:</td>
                                        <td>{{ getFormatTanggal(detail.items[0].manajer_tgl) }} -  {{ detail.items[0].manajer_nik }}</td>
                                    </tr>
                                    <tr>
                                        <td>Konfirmasi Tiering</td>
                                        <td>:</td>
                                        <td>{{ getFormatTanggal(detail.items[0].tiering_tgl) }} -  {{ detail.items[0].tiering_nik }}</td>
                                    </tr>
                                    <tr>
                                        <td>Grand Total</td>
                                        <td>:</td>
                                        <td><strong> Rp. {{ getFormatNumber(getGrandTotal) }}</strong></td>
                                    </tr>
                                </tbody>
                            </v-simple-table>
        ...
        <v-col>
            <v-card outlined>
                <v-card-text class="pa-0">
                    <v-data-table :headers="detail.headers" :items="detail.items" :loading="detail.loading"
                        ...
                        <template v-slot:[`item.nama_bahan`]="{ item }">
                            <v-chip  small outlined label>{{ item.nama_bahan }}, {{ item.ukuran }}</v-chip>
                            <!-- <v-chip class="my-1" small outlined label>{{ item.ukuran }}</v-chip> -->
                             <v-chip  x-small color="" outlined label>{{ item.kode_bahan }}</v-chip>
                        ...
                        <template v-slot:[`item.status_pakai`]="{ item }">
                            <v-chip label :color="getColorStaPakai(item.status_pakai)" outlined small>{{
                                    item.status_pakai
                            }}</v-chip>

                        </template>
                        <template v-slot:[`item.subtotal`]="{ item }">
                            <strong>{{ getFormatNumber(item.jumlah * item.harga_perkiraan) }}</strong>
                        </template>
                    </v-data-table>
                </v-card-text>
            </v-card>
...
export default {
        ...
        return {
            ...
            detail: {
                headers: [
                    ...
                    { text: 'Jenis Biaya', value: "jenis_biaya" },
                    { text: 'Sub Jenis Biaya', value: "subjenis_biaya" },
    ...
    methods: {
        ...
        updateKonfirmasi(status) {
            axios.post('konfirmasi/data/update', {
        ...
        async loadDetail(nousulan) {
            this.resetDetail()
            await axios.post('pembelian/usulanjasa/bynousulan', {
                'no_usulan': nousulan
            })
                .then((res) => {
                    if (res.data.success == false) {
    ...
    computed: {
        getGrandTotal() {
            return this.detail.items.reduce((acc, item) => acc + (item.jumlah * item.harga_perkiraan) , 0)
        }
    }
