<template>
  <v-container>
    <v-row>
      <v-col>
        <div class="text-h5">MASTER SUPPLIER</div>
      </v-col>
    </v-row>
    <v-divider></v-divider>
    <!-- <v-row class="mt-2">
              ...
              >
                <template v-slot:[`item.opsi`]="{ item }">
                  <v-btn
                    color="primary"
                    text
                    @click="pilihSupplier(item.kode, item.nama)"
                    v-if="staPilih"
...
export default {
      ...
      addData: {
        ...
        rules: {
          nama: [
            ...
            (v) => v.length > 3 || "Nama Harus Lebih dari 3 Karakter",
          ],
          alamat: [
            (v) => v != "" || "Alamat Harus Diisi",
            (v) => v.length > 3 || "Alamat Harus Lebih dari 3 Karakter",
          ],
          telepon: [
            (v) => v != "" || "Telepon Harus Diisi",
            (v) => v.length > 3 || "Telepon Harus Lebih dari 3 Karakter",
          ],
          person: [
            (v) => v != "" || "Person Harus Diisi",
            (v) => v.length > 3 || "Person Harus Lebih dari 3 Karakter",
          ],
        },
        loading: false,
      },
  ...
  methods: {
    ...
    async createData() {
      ...
      await axios
        .post("master/supplier/create", this.addData.form)
        .then((res) => {
          if (res.status != 200) {
    ...
    async updateData() {
      console.log(this.addData.form);
      await axios
        .post("master/supplier/update", this.addData.form)
        .then((res) => {
          swal("Sukses", res.data.message, "success");
          this.addData.dialog = false;
          this.resetForm();
    ...
    pilihSupplier(kode, nama) {
      this.$emit("supplier", { kode: kode, nama: nama });
    },
  },
};
</script>
